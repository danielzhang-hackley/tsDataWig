...
np.random.seed(0)
DIR_PATH = 'C:\\Users\\danie\\miniconda3\\envs\\regeneron_project_2\\running files'
...
def impute_datawig(X, mask):
    ...
    for d in glob.glob(os.path.join(dw_dir)):
        # TODO: figure out how to remove the directories
        # shutil.rmtree(d)
        pass
...
def impute_median(x, mask):
    return fancyimpute_hpo(SimpleFill, {'fill_method': ["median"]}, x, mask)
...
def impute_rf(x, mask):
    ...
    x_pred = IterativeImputer(random_state=0, predictor=reg).fit_transform(x_incomplete)  # figure out difference

    return x_pred


def impute_lr(x, mask):
    x_incomplete = x.copy()
    x_incomplete[mask] = np.nan
...
def get_data(data_fn):
    if data_fn.__name__ == 'make_low_rank_matrix':
        X = data_fn(n_samples=1000, n_features=10, effective_rank=5, random_state=0)
    elif data_fn.__name__ == 'make_swiss_roll':
...
def experiment(percent_missing_list=[5, 10, 30, 50], nreps=3):
    DATA_LOADERS = [
        make_low_rank_matrix,
        load_diabetes,
        load_wine,
    ...
    with open(os.path.join(DIR_PATH, 'benchmark_results.json'), 'w') as fh:
        for percent_missing in tqdm(percent_missing_list):  # ???
            for data_fn in DATA_LOADERS:
                X = get_data(data_fn)
                for missingness in ['MAR', 'MCAR', 'MNAR']:
